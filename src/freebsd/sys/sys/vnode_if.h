...
#include "sys/vnode_if_typedef.h"
#include "sys/vnode_if_newproto.h"
...
int VOP_UNLOCK_AP(struct vop_unlock_args *args);
int VOP_UNLOCK_APV(struct vop_vector *vop, struct vop_unlock_args * args);
...
int VOP_BMAP_AP(struct vop_bmap_args * args);
int VOP_BMAP_APV(struct vop_vector *vop, struct vop_bmap_args * args);
...
int VOP_GETPAGES_AP(struct vop_getpages_args * args);
int VOP_GETPAGES_APV(struct vop_vector *vop, struct vop_getpages_args * args);
...
int VOP_SET_TEXT_AP(struct vop_set_text_args * args);
int VOP_SET_TEXT_APV(struct vop_vector *vop, struct vop_set_text_args * args);
...
#if !defined(DEBUG_VFS_LOCKS) && !defined(INVARIANTS) && !defined(KTR)
	if (!SDT_PROBES_ENABLED())
		return (vp->v_op->vop_fdatasync(&a));
	else
