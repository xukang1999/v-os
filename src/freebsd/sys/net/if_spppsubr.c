...
__begin_packed struct ppp_header {
	u_char address;
	u_char control;
	u_short protocol;
} __packed;
...
__begin_packed struct lcp_header {
	u_char type;
	u_char ident;
	u_short len;
} __packed;
...
__begin_packed struct cisco_packet {
	u_long type;
	u_long par1;
	u_long par2;
	u_short rel;
...
static void sppp_cisco_send(struct sppp *sp, int type, long long par1, long long par2);
	...
	struct route *ro)
{
	...
	if (dst->sa_family == AF_INET) {
		...
		struct tcphdr *tcp = (struct tcphdr*) ((long long*)ip + ip->ip_hl);
 ...
 */
static void
sppp_cisco_send(struct sppp *sp, int type, long long par1, long long par2)
{
	STDDCL;
	struct ppp_header *h;
...
sppp_cp_input(const struct cp *cp, struct sppp *sp, struct mbuf *m)
{
	...
	case ECHO_REQ:
		...
		if ((sp->lcp.opts & (1 << LCP_OPT_MAGIC)) &&
		    ntohl (*(long long*)(h+1)) == sp->lcp.magic) {
		...
		*(long long*)(h+1) = htonl (sp->lcp.magic);
		if (debug)
