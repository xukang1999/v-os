...
static long long time_tai;			/* TAI offset (s) */
static long long time_monitor;		/* last time offset scaled (ns) */
static long long time_constant;		/* poll interval (shift) (s) */
static long long time_precision = 1;		/* clock precision (ns) */
static long long time_maxerror = MAXPHASE / 1000; /* maximum error (us) */
long long time_esterror = MAXPHASE / 1000; /* estimated error (us) */
static long long time_reftime;		/* uptime at last adjustment (s) */
...
static long long pps_stabil;			/* nominal stability (scaled ns/s) */
static long long pps_lastsec;		/* time at last calibration (s) */
 ...
 */
static long long pps_calcnt;			/* calibration intervals */
static long long pps_jitcnt;			/* jitter limit exceeded */
static long long pps_stbcnt;			/* stability limit exceeded */
static long long pps_errcnt;			/* calibration errors */
#endif /* PPS_SYNC */
/*
...
static void hardupdate(long long offset);
...
static void
hardupdate(offset)
long long offset;		/* clock offset (ns) */
{
	long long mtemp;
	l_fp ftemp;
...
hardpps(struct timespec *tsp, long nsec)
{
	...
	*/
	if (u_nsec > lmax(pps_jitter << PPS_POPCORN,
	    2 * (NANOSECOND / (long long)qmin(NANOSECOND, tc_getfrequency())))) {
		time_status |= STA_PPSJITTER;
		pps_jitcnt++;
